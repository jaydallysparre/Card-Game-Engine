...
class PlayerInputState: public State {
    ...
    // Progress is made only after the recieving the player input.
    void run() {}
    // When the player presses a button
    void handleEvent(std::unique_ptr<RequestEvent> ev){
        // Variable to store the buttonId that was pressed from event
        ObjectId pressedButton;
        auto& HiLoScene = static_cast<HiLoScene&>(scene);
        ...
        scene.moveCard(*topCard0, deckIds[0], deckIds[2]);
